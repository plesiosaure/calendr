       ...
       $possible_match = is_null($method_match) || $method_match;
        ...
        // Easily handle 404's
            ...
            try {
	            ...
	            $data = call_user_func_array($callback, array($request, $response, $app, $matched, $methods_matched));
            } catch (Exception $e) {
                $response->error($e);
            }
        ...
        if (isset($match) && $match ^ $negate) {
             ...
             if ($possible_match) {
				...
				#	$_REQUEST = array_merge($_REQUEST, $params);
                  ...
                  try {
                  #  $d = call_user_func($callback, $request, $response, $app, $matched, $methods_matched);
                    $data = call_user_func_array($callback, array($request, $response, $app, $matched, $methods_matched));
