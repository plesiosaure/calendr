...
class datamongo extends coreApp{
	...
	static  function mongoCon(){
		...
		if (self::$mongoDB === NULL){
			...
			try {
				...
				$timeout    = !empty($config['mongodb']['timeout']) ? $config['mongodb']['timeout'] : 5000;
				$port       = !empty($config['mongodb']['port'])    ? $config['mongodb']['port']    : 27017;
