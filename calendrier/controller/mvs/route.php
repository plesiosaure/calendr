...
respond(function ($request, $response, $router) {
	global $app;
	$router->app = $app;
#	$router->app->token = $request->header('WF-TOKEN');
});
...
respond('GET', '/manifestation/[id|_id:method]/[:id]', function($request, $response, $router){
...
respond('POST', '/manifestation/([id|_id:method])/[:id]', function($request, $response, $router){
